# Network
### What is VPC、region、avaliable zones(az), subnet、route table、internet gateway、nat gateway。

Short Answer
- Region  
  AWS 在全球的實體區域（如 `ap-northeast-1`、`eu-west-2`）。  
  多數資源以 Region 為範圍，不同 Region 彼此隔離。

- Availability Zone (AZ)
  Region 內的獨立機房群（如 `ap-northeast-1a`、`1c`）。  
  用來實現高可用，Subnet 必須屬於某一個 AZ。

- VPC (Virtual Private Cloud)
  你在 AWS 上的「私有網路」，自訂 IP 範圍（CIDR），  
  所有 Subnet、路由與資源都在 VPC 內運作。

- Subnet
  VPC 的子網段，綁定一個 AZ。  
  是否為 Public / Private 由綁定的 Route Table 決定。

- Route Table
  決定封包「要往哪裡走」的規則表。  
  例如：`0.0.0.0/0 → IGW` 或 `0.0.0.0/0 → NAT`

- Internet Gateway (IGW)
  VPC 對外的出口，  
  讓 Public Subnet 的資源能直接連上 Internet。

- NAT Gateway
  讓 Private Subnet 內的資源「可以主動連外」，  
  但「不能被外界連入」，用於內網安全上網。
...
> 遇到「不能新增 `0.0.0.0/0 → NAT`」：原因是在 **public-rt**
> 裡試圖新增第二條 `0.0.0.0/0`（Destination 重複不允許）。\
> NAT 的 `0.0.0.0/0 → nat-xxxx` 必須加在 **private-rt**。

------------------------------------------------------------------------

## 4) 建兩台 EC2（Public / Private）＋從 Laptop SSH 連上兩台
...
-   Name：`sg-public-ec2`
-   VPC：**一定要選 `test-vpc-1`**（SG 不出現就是 VPC 不同）
...
> 出現 `Permission denied (publickey)`：Ubuntu 的 username 是
> `ubuntu`，不是 `ec2-user`。
...
![nginx](./Resource/Image/nginx.png)

> 若看不到：通常是 SG 沒開 HTTP 80、或打成 https。
...
curl https://www.google.com
```
![google](./Resource/Image/curlgoogle.png)
...
完整踩坑可看 [full](Resource/Network/aws-vpc-ec2-nat-lab-full.md)
------------------------------------------------------------------------